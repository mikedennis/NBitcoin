...
using Xunit;
using static NBitcoin.Altcoins.Stratis;
	...
	public class AltcoinTests
	{
		...
		public void CanSignTransactions()
		{
			using (var builder = NodeBuilderEx.Create())
			{
				...
				this.EnsureWalletUnlocked(node, rpc);
		...
		private void EnsureWalletUnlocked(CoreNode node, RPCClient rpc)
		{
			var stratis = NBitcoin.Altcoins.AltNetworkSets.Stratis.Regtest;
			if (node.Network == stratis)
			{
				rpc.WalletPassphrase("default", 300);
			}
		}

		private void EnsureWallet(CoreNode node)
		{
			var stratis = NBitcoin.Altcoins.AltNetworkSets.Stratis.Regtest;
			if (node.Network == stratis)
			{
				var destPath = Path.Combine(node.Folder, "data/stratis/StratisRegTest/stratis.wallet.json");
				Directory.CreateDirectory(Path.GetDirectoryName(destPath));
				File.Copy("data/stratis.wallet.json", destPath);
			}
		}
	}
}